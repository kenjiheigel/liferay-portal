...
A bump is only owed against the last release, so for an `*-api` module check the release before failing. The module's `modules/.releng/<path>/artifact.properties` records the commit that release was built from as `artifact.git.id`, where `<path>` is the module path below `modules/`. The package needs no bump in the diff when the module has no `artifact.properties`, since it was never released, when the package has no `packageinfo` at that commit, since the package has never shipped, or when its `packageinfo` in the tree is already above the one at that commit, since an earlier commit bumped it and the release has not gone out yet. Read the commit with `git show <artifact.git.id>:<packageinfo path>` rather than the network, so the check still runs offline. `portal-impl` and `portal-kernel` have no `artifact.properties`, so keep the diff rule for them.
